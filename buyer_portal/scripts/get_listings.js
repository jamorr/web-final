...
function load_listings() {
	...
	xhr.open("GET", `read_listings.php?offset=${current_offset}&limit=${limit}` + search_terms);
	xhr.onload = function() {
...
function search() {
	search_terms = "";
	if (query.value != "")
		search_terms += `&query=${query.value}`;
	if (min_price.value != "")
		search_terms += `&min_price=${min_price.value}`;
	if (max_price.value != "")
		search_terms += `&max_price=${max_price.value}`;
	if (min_bed.value != "")
		search_terms += `&min_bed=${min_bed.value}`;
	if (max_bed.value != "")
		search_terms += `&max_bed=${max_bed.value}`;
	if (min_bath.value != "")
		search_terms += `&min_bath=${min_bath.value}`;
	if (max_bath.value != "")
		search_terms += `&max_bath=${max_bath.value}`;
